...
// modified version of code found @ https://thehiddensignal.com/unity-angle-of-sloped-ground-under-player/ (finally)!!
public class GroundChecker : MonoBehaviour
{
    ...
    public RaycastHit rearSlopeHit; //how we measure if we are looking up the hill or down, otherwise the angle is always the same
    public RaycastHit frontSlopeHit;
    ...
    public void CheckGround(Vector3 origin)
    {
        ...
        //RaycastHit rearSlopeHit;
        //RaycastHit frontSlopeHit;

        // FIRST RAYCAST
        if (Physics.Raycast(origin + rayOriginOffset1, Vector3.down, out rearSlopeHit, raycastLength))
        {
            // Debug line to first hit point
            if (showDebug) { Debug.DrawLine(origin + rayOriginOffset1, rearSlopeHit.point, Color.red); }
            // Get angle of slope on hit normal
            rearAngle = Vector3.Angle(rearSlopeHit.normal, Vector3.up);

            // 2ND RAYCAST
            if (Physics.Raycast(origin + rayOriginOffset2, Vector3.down, out frontSlopeHit, raycastLength))
            {
                // Debug line to second hit point
                if (showDebug) { Debug.DrawLine(origin + rayOriginOffset2, frontSlopeHit.point, Color.blue); }
                // Get angle of slope of these two hit points.
                frontAngle = Vector3.Angle(frontSlopeHit.normal, Vector3.up);
                // 3 collision points: Take the MEDIAN by sorting array and grabbing middle.
                float[] tempArray = new float[] { groundSlopeAngle, rearAngle, frontAngle };
            ...
            else
            {
                ...
                float average = (groundSlopeAngle + rearAngle) / 2;
